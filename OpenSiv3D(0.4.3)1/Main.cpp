...
void Main()
{
	...
	while (System::Update()) {
		...
		circlePos.first += abs(velocity.first) == INF ? 0 : velocity.first;
		...
		if (velocity.second < 0) velocity.second = -1 * velo;
			...
			else velocity.first = INF;
			if (velocity.second < 0) velocity.second = -1 * INF;
			else velocity.second = INF;
		}
		...
		hitSlider = Rect(10, 550, 350, 36);			//スライダーの当たり判定
